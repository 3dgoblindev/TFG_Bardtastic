...
\chapter{Utilidad, soluciones y equilibrio}\label{cap2}
...
\begin{teorema}[Teorema del punto fijo de Kakutani, {\cite[Teorema~3.2]{TeoriaDeJuegosCJP}}]
...
\begin{teorema}[Existencia de equilibrio de Nash en juegos continuos {\cite[Teorema~3.3]{TeoriaDeJuegosCJP}}]
	Sea \( G = \{S_1,\dots,S_n; u_1,\dots,u_n\} \) un juego en el que, para cada jugador \( i \), se cumple que:
	\begin{itemize}
		\item \( S_i \) es un subconjunto no vacío, compacto y convexo de un espacio \(\mathbb{R}^k\),
		\item la función de utilidad \( u_i \) es continua en todo su dominio \( S = S_1 \times \cdots \times S_n \) y es cuasicóncava en la variable \( s_i \).
	\end{itemize}
...
\begin{proof}
	...
	Para cada jugador \( i\in J \), el conjunto de estrategias \( S_i \) es no vacío, compacto y convexo, y la función de utilidad \( u_i : S \to \mathbb{R} \) es continua en \( S \) y cuasicóncava en la variable \( s_i \).
	
	Para cada jugador definimos su correspondencia de \textit{respuesta óptima}:
	\[
	R_i(s) = \left\{ x_i \in S_i \,\middle|\, u_i(s_1,\dots,x_i,\dots,s_n) \ge
	u_i(s_1,\dots,y_i,\dots,s_n), \ \forall y_i \in S_i \right\}.
	\]
	
	Definimos la correspondencia global:
	\[
	R(s) = R_1(s) \times \cdots \times R_n(s).
	\]
	
	A continuación comprobamos que \(R\) cumple las hipótesis del teorema del punto fijo de Kakutani:
	
	\begin{enumerate}
		\item \textbf{El conjunto \( S \) es no vacío, compacto y convexo.}  
		Estas propiedades se preservan bajo el producto cartesiano.
		
		\item \textbf{\(R(s)\) es no vacío para todo \(s\).}  
		Como \(u_i\) es continua en el compacto \(S_i\), el máximo existe por Weierstrass, por lo que \(R_i(s) \neq \varnothing\), y en consecuencia \(R(s)\neq\varnothing\).
		
		\item \textbf{Cada \( R(s) \) es convexo.}  
		La cuasicóncavidad de \(u_i\) en \(s_i\) asegura que el conjunto de maximizadores \(R_i(s)\) es convexo. El producto cartesiano de conjuntos convexos también lo es.
		
		\item \textbf{La correspondencia \(R\) es hemicontinua superiormente.}  
		Si \(s^k \to s\), \(t^k \in R(s^k)\) y \(t^k \to t\), al pasar al límite y usar la continuidad de \(u_i\) se obtiene \(t_i \in R_i(s)\) para todo \(i\).  
		Por tanto, el grafo de \(R\) es cerrado.
	\end{enumerate}
	
	Como \(R\) satisface todas las condiciones del teorema del punto fijo de Kakutani, existe \(s^* \in S\) tal que \(s^* \in R(s^*)\).  
	Esto implica que cada componente \(s_i^*\) es respuesta óptima frente a \(s^*\), y por tanto \(s^*\) es un \textbf{equilibrio de Nash en estrategias puras}.
...
\begin{teorema}[Existencia de equilibrio de Nash en juegos finitos {\cite[Teorema~3.4]{TeoriaDeJuegosCJP}}] \label{T_mixtas}
...
\begin{teorema}[Equilibrio de Nash simétrico {\cite[Teorema~3.5]{TeoriaDeJuegosCJP}}]
...
\begin{teorema}[Teorema del minimax {\cite[Teorema~3.6]{TeoriaDeJuegosCJP}}] \label{T_minimax_neumann}
...
\begin{teorema}[Equilibrio y estrategias \textit{minimax} {\cite[Teorema~3.7]{TeoriaDeJuegosCJP}}] \label{T_minimax_equilibrio}
