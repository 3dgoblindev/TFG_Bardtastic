...
\section{Agentes}
En inteligencia artificial, un agente es una entidad que percibe su entorno mediante sensores y actúa sobre él a través de actuadores \cite{RussellNorvig2021}. El comportamiento del agente puede describirse mediante una regla de decisión (a veces llamada función ''agente``), que indica qué acción debe realizar el agente en cada situación posible que pueda percibir.
...
\needspace{8\baselineskip} % reserva sitio para que no se parta
\begin{itemize}[leftmargin=1.8em, itemsep=2pt, topsep=2pt, parsep=0pt, partopsep=0pt]
	\item En el caso MAX: cuando calculamos un sucesor \(s\) y obtenemos un valor
	\(v = \text{AlphaBeta}(s, \alpha, \beta)\), si \(v > \alpha\) actualizamos
	\(\alpha = v\). Si en algún momento \(\alpha \ge \beta\), rompemos el bucle de sucesores (corte).
	\item En el caso MIN: cuando obtenemos un sucesor con valor \(v\), si \(v < \beta\)
	actualizamos \(\beta = v\). Si \(\beta \le \alpha\), hacemos corte (podar).
\end{itemize}
...
\paragraph{El algoritmo de búsqueda $\alpha-\beta$}
...
Las flechas mostradas en la Figura \ref{fig:tres_en_raya_alphabeta} representan el recorrido de la búsqueda durante la evaluación $\alpha$–$\beta$. Indican cómo el algoritmo avanza y retrocede entre nodos, propagando valores hacia arriba y permitiendo identificar cuándo una rama deja de ser relevante para la decisión final. Esta visualización ayuda a comprender cómo se van actualizando $\alpha$ y $\beta$ y por qué ciertas ramas pueden podarse sin necesidad de explorarlas por completo.
...
El entrenamiento mediante \textit{self-play} permitió que los agentes actualizaran sus políticas tras cada simulación. Los resultados mostraron que tanto el UCT estándar como \textit{Smooth UCT} alcanzaron políticas competitivas frente a \textit{bots} de referencia, aproximando comportamientos cercanos al equilibrio de Nash. Aunque no se demostró formalmente la convergencia teórica en juegos con información imperfecta, los resultados empíricos evidencian que MCTS es capaz de generar estrategias robustas mediante aprendizaje autónomo y simulación masiva.
